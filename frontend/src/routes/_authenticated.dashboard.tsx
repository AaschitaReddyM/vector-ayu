...
function DashboardPage() {
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-7 py-7 flex flex-col gap-6">
      <div className="animate-fade-up flex justify-between items-start">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3">
          <select 
            ...
            className="bg-card border border-border text-text px-4 py-2 rounded focus:outline-none focus:border-teal"
          >
            {patients.map(p => (
          ...
          <button
            className={`px-4 py-2 rounded font-semibold transition ${showSandbox ? 'bg-amber text-background' : 'bg-card border border-border text-amber hover:bg-amber/10'}`}
            ...
            onClick={async (e) => {
              ...
              try {
                ...
                if (res.ok) {
                  ...
                  // Inject the newly calculated RiskScore into the local frontend cache!
                  ...
                  // The triage queue will refetch from our updated backend state
                  await queryClient.invalidateQueries({ queryKey: ["triage_queue"] });
                  toast.success(`Pipeline executed for ${selectedPatientId}`, { id: "sim" });
                } else {
          ...
          <button
            className="bg-card border border-border text-text-dim px-4 py-2 rounded font-semibold hover:text-teal transition"
            onClick={async (e) => {
              e.preventDefault();
              try {
                ...
                if (res.ok) {
                  ...
                  toast.error(`Cron failed: ${res.statusText}`, { id: "cron" });
                }
              } catch (err) {
                console.error("Cron failed", err);
                toast.error("Network error during cron", { id: "cron" });
              }
            }}
          >
            Simulate Cron (Live Data)
          </button>
          
          <div className="flex border border-coral/50 rounded overflow-hidden shrink-0">
            <select
              value={anomalyType}
              onChange={(e) => setAnomalyType(e.target.value)}
              className="bg-coral/5 text-coral px-3 py-2 text-sm focus:outline-none focus:bg-coral/10 transition-colors border-r border-coral/50 cursor-pointer"
            >
              {typeof window !== 'undefined' && localStorage.getItem('vayu_region') === 'new_delhi' ? (
                <>
                  <option value="respiratory">Respiratory 🫁 (Crop Stubble Smog)</option>
                  <option value="cardiovascular">Cardiovascular ❤️ (Pre-Monsoon Heatwave)</option>
                  <option value="metabolic">Metabolic ⚡ (Monsoon Flash Flooding)</option>
                </>
              ) : (
                <>
                  <option value="respiratory">Respiratory 🫁 (Wildfire Smoke Plume)</option>
                  <option value="cardiovascular">Cardiovascular ❤️ (Extreme Heat Dome)</option>
                  <option value="metabolic">Metabolic ⚡ (ERCOT Power Grid Failure)</option>
                </>
              )}
            </select>
            <button
              className="bg-coral/10 text-coral px-4 py-2 font-semibold hover:bg-coral hover:text-white transition flex items-center gap-2 whitespace-nowrap shrink-0"
              onClick={async (e) => {
                e.preventDefault();
                try {
      ...
      {/* Clinical Sandbox Panel (Inline) */}
      {showSandbox && (
        <div className="bg-amber/5 border border-amber/30 rounded-xl p-5 animate-fade-up">
          <h3 className="font-bold text-amber flex items-center gap-2 mb-4">
            🧪 What-If Simulator Overrides
            <span className="text-xs text-text-dim font-normal">Values will be injected when you click "Run Live Simulation"</span>
          </h3>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="text-xs font-bold uppercase text-text-dim mb-2 block">Override SpO2 (%)</label>
              <input type="number" placeholder="e.g. 88" className="w-full bg-black/40 border border-border rounded px-4 py-2 text-sm focus:border-amber focus:outline-none transition-colors" 
            ...
            <div>
              ...
              <input type="number" placeholder="e.g. 150" className="w-full bg-black/40 border border-border rounded px-4 py-2 text-sm focus:border-amber focus:outline-none transition-colors" 
            ...
            <div>
              ...
              <input type="number" placeholder="e.g. 165" className="w-full bg-black/40 border border-border rounded px-4 py-2 text-sm focus:border-amber focus:outline-none transition-colors" 
